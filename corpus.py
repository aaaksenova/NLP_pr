...
class Corpus():
        ...
        for sent_ind, sent in self.data.iterrows():
            if pattern in sent['sentence'].split():
                ...
                changed_sent = re.sub(r'(\b|^)' + pattern + r'(\b|$)',
                                       ...
                                       sent['sentence'])
                indeces.append((sent_ind, word_ind, changed_sent))
    ...
    def analyze(self, pattern, column, indeces):
       ...
       new_indeces = []
       answers = []
       for sent_id, first_word_id, sentence in indeces: # индекс предложения, индекс слова в предложении
          answer = self.data[column][sent_id].split() # строка, по которой ведется поиск
          word_id = first_word_id + 1 # индекс слова, которое мы ищем
          if word_id < len(answer) and answer[word_id] == pattern: # соответсвует ли слово паттерну
              changed_sent = re.sub(r'(\b|^)' + self.data['tokenized'][sent_id].split()[word_id] + r'(\b|$)', 
                                     '\033[0;34;47m' + self.data['tokenized'][sent_id].split()[word_id] + '\033[0m', 
                                     sentence)
              new_indeces.append((sent_id, word_id, changed_sent)) # добавляет индекс предложения и индекс слова, которое нашли
              answers.append((pattern, # паттерн
                        self.data['source'][sent_id], # источник
                        self.data['title'][sent_id], # название статьи
    ...
    def check_next_word(self, pattern, indeces, is_last=False):
      ...
      if '+' in pattern: # поиск знать+NOUN, поиск одновременно по леммам и частям речи, поэтому не вызывает analyze()
          for sent_id, old_id, sentence in indeces:
              ...
              if word_id < len(answer) and word in answer[word_id].split('|') and pos_tag in pos[word_id].split('|'):
                  new_indeces.append((sent_id, word_id, changed_sent))
                  token = self.data['tokenized'][sent_id].split()[word_id]
                  changed_sent = re.sub(r'(\b|^)' + token + r'(\b|$)', 
                                       '\033[0;34;47m' + token + '\033[0m', 
                                       sentence)
                  answers.append((pattern,
                                 self.data['source'][ind], 
                                 self.data['title'][ind],
                                 changed_sent))
